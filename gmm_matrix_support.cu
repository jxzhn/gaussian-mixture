 ...
 * @param n 
 */
__global__ void rowSumKernel(const double* mat, int m, int n, double* tmp, int orig_n) {
    __shared__ double sdata[BLOCK_DIM_1D];
    int tid = threadIdx.y;
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (tid == 0)
    {
        tmp[i * orig_n + blockIdx.y] = sdata[0];
    }

}
__global__ void rowSumRes(double* buf, int m, int n, double* tmp) {
    for(int i = 0; i < m; i++)
    {
        buf[i] = tmp[i * n];
    }
...
# endif
    ...
    dim3 gridSize(grid_DIM_1D, grid_DIM_2D);
    rowSumKernel<<<gridSize, blockSize>>>(mat, m, n, tmp, n); 
    ...
    while(lastGrid_DIM_2D > 1)
    {
        ...
        dim3 gridSize(grid_DIM_1D, grid_DIM_2D);
        rowSumKernel<<<gridSize, blockSize>>>(tmp, m, lastGrid_DIM_2D, tmp, n);
        lastGrid_DIM_2D = grid_DIM_2D;
    }
    rowSumRes<<<1, 1>>>(buf, m, n, tmp);
